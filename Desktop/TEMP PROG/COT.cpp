...
int N,M,v[MXN],rv[MXN],tab[MXN][int(log2(MXN))],lvl[MXN];
...
void bd(tr *cr,int s,int e){ // BUILD TREE
	if(s==e) return;
	int m=(s+e)>>1;
	cr->l=new tr();
	cr->r=new tr();
	bd(cr->l,s,m); bd(cr->r,m+1,e);
}
...
/*int gt(tr cr,int x){ // QUERY SEGMENT TREE
	int ret=0;
	for(int s=0,e=N;s<e;){
		int m=(s+e)>>1;
		if(x<=m) cr=*cr.l,e=m;
		else ret+=cr.l->w, cr=*cr.r, s=m+1;
		if(s==e) ret+=cr.w;
	}
	return ret;
}*/
